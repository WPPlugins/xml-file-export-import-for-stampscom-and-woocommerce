...
class WF_OrderImpExpStampsXML_Order_Import extends WP_Importer {
        ...
        switch ($step) {
            ...
            case 1 :
                ...
                if (!empty($_GET['clearmapping']) || $this->handle_upload())
                    ...
                    _e('Error with handle_upload!', 'wf_order_import_export_stamps_xml');
                break;
            case 2 :
                $this->header();
                ...
                if ($file) {
                    ...
                    <table id="import-progress" class="widefat_importer widefat">
                        <thead>
                            <tr>
                                <th class="status">&nbsp;</th>
                                <th class="row"><?php _e('Row', 'wf_order_import_export_stamps_xml'); ?></th>
                                <th><?php _e('OrderID', 'wf_order_import_export_stamps_xml'); ?></th>
                                <th><?php _e('Processed', 'wf_order_import_export_stamps_xml'); ?></th>
                                <th class="reason"><?php _e('Status Msg', 'wf_order_import_export_stamps_xml'); ?></th>
                            </tr>
                        </thead>
                        <tfoot>
                                ...
                                try {
                                                ...
                                                $('#import-progress tbody').append('<tr id="row-' + i + '" class="error"><td class="status" colspan="5">' + results.error + '</td></tr>');
                                                                            ...
                                                                            $('#import-progress tbody').append('<tr id="row-' + i + '" class="' + row['status'] + '"><td><mark class="result" title="' + row['status'] + '">' + row['status'] + '</mark></td><td class="row">' + i + '</td><td>' + row['order_number'] + '</td><td>' + row['post_id'] + ' - ' + row['post_title'] + '</td><td class="reason">' + row['reason'] + '</td></tr>');
                                                                                                        ...
                                                                                                        $('#import-progress tbody').append('<tr class="error"><td class="status" colspan="5">' +  '<?php _e('AJAX Error', 'wf_order_import_export_stamps_xml'); ?>' + '</td></tr>');
                                                                                                                                    }
                    ...
                    echo '<p class="error">' . __('Error finding uploaded file!', 'wf_order_import_export_stamps_xml') . '</p>';
                }
                break;
            case 3 :
            ...
            case 4 :
                ...
                _e('Step 1...', 'wf_order_import_export_stamps_xml') . ' ';

                wp_defer_term_counting(true);
                wp_defer_comment_counting(true);

                _e('Step 2...', 'wf_order_import_export_stamps_xml') . ' ';

                echo 'Step 3...' . ' '; // Easter egg

                _e('Finalizing...', 'wf_order_import_export_stamps_xml') . ' ';

                // SUCCESS
                _e('Finished. Import complete.', 'wf_order_import_export_stamps_xml');
    ...
    public function import() {
        ...
        $this->log->add('stamps-import', '---');
        $this->log->add('stamps-import', __('Processing orders.', 'wf_order_import_export_stamps_xml'));
        $merging = 1;
        $record_offset = 0;
        ...
        $this->log->add('stamps-import', __('Finished processing Orders.', 'wf_order_import_export_stamps_xml'));
    ...
    public function import_start($file) {
        ...
        $this->log->add('stamps-import', '---[ New Import ] PHP Memory: ' . $memory . ', WP Memory: ' . $wp_memory);
        $this->log->add('stamps-import', __('Parsing order XML.', 'wf_order_import_export_stamps_xml'));

        $this->parser = new WF_XML_Parser('shop_order');

        $this->parsed_data = $this->parser->parse_data($file);

        $this->log->add('stamps-import', __('Finished parsing order XML.', 'wf_order_import_export_stamps_xml'));
    ...
    public function handle_upload() {
        ...
        if (empty($_POST['file_url'])) {
            ...
            if (isset($file['error'])) {
                echo '<p><strong>' . __('Sorry, there has been an error.', 'wf_order_import_export_stamps_xml') . '</strong><br />';
            ...
            if (file_exists(ABSPATH . $_POST['file_url'])) {
                ...
                echo '<p><strong>' . __('Sorry, there has been an error.', 'wf_order_import_export_stamps_xml') . '</strong></p>';
                return false;
            }
        }
    ...
    private function process_orders($post) {
        ...
        $this->add_import_result($view_status, __($out_updated_msg, 'wf_order_import_export_stamps_xml'), $post['order_number'], $post['order_number'], $post['order_number']);
        $this->log->add('stamps-import', sprintf(__('> &#8220;%s&#8221;' . $out_updated_msg, 'wf_order_import_export_stamps_xml'), $post['order_number']), true);
        $this->log->add(sprintf(__('> Finished importing order %s', 'wf_order_import_export_stamps_xml'), $post['order_number']));


        $this->log->add(__('Finished processing orders.', 'wf_order_import_export_stamps_xml'));
    ...
    public function header() {
        ...
        echo '<h2>' . ( empty($_GET['merge']) ? __('Import', 'wf_order_import_export_stamps_xml') : __('Merge Orders', 'wf_order_import_export_stamps_xml') ) . '</h2>';
    }
